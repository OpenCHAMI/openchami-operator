...
const (
	...
	// testTagV1 is the throwaway tag the image-pinning webhook tests use
	// in their fixtures. Extracted as a constant because the same value
	// recurs across negative + positive test cases and the linter
...
// newFixtureCluster returns a minimally-valid OpenCHAMIControlPlane suitable for
...
func newFixtureCluster(name string) *OpenCHAMIControlPlane {
	return &OpenCHAMIControlPlane{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: "default",
			UID:       types.UID(name + "-uid"),
		},
		Spec: OpenCHAMIControlPlaneSpec{
			ClusterName: name,
...
func newWebhook(t *testing.T, existing ...client.Object) *OpenCHAMIControlPlaneWebhook {
	t.Helper()
	scheme := newWebhookScheme(t)
	c := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existing...).Build()
	return &OpenCHAMIControlPlaneWebhook{Client: c}
}
...
func TestDefault_AppliesAllDerivedDefaults(t *testing.T) {
	...
	c := &OpenCHAMIControlPlane{
		Spec: OpenCHAMIControlPlaneSpec{ClusterName: "alpha"},
	}
...
func TestValidateCreate_ImagesPinnedRequiresMatchingEntries(t *testing.T) {
	...
	c.Spec.Images.Stream = ImageStreamPinned
	// Tokensmith intentionally missing — webhook must surface it.
	c.Spec.Images.Pinned = map[string]string{
		pinKeySMD: testTagV1,
	}
	_, err := w.ValidateCreate(context.Background(), c)
	expectInvalid(t, err, "images.pinned")
	// Error message should name the missing service so the user knows
	// exactly what to add — not just "something is missing".
	if err == nil || !strings.Contains(err.Error(), pinKeyTokensmith) {
		t.Errorf("expected error to name tokensmith, got: %v", err)
	}
}

func TestValidateCreate_ImagesPinnedAcceptsCompleteMap(t *testing.T) {
	w := newWebhook(t)
	c := newFixtureCluster("a")
	// Fixture has CoreDHCP+Magellan enabled and networkProbe enabled
	// (no nodeSelector needed). Enable SMD/Tokensmith/Boot/Metadata so
	// the pinned-map check has to find them all.
	c.Spec.Services.SMD.Enabled = true
	c.Spec.Services.Tokensmith.Enabled = true
	...
	c.Spec.Images.Pinned = map[string]string{
		pinKeySMD:             testTagV1,
		pinKeyTokensmith:      "v0.4.0",
		pinKeyBootService:     "v0.3.0",
		pinKeyMetadataService: "v0.5.0",
		pinKeyCoreDHCP:        "v0.1.0",
		pinKeyMagellan:        "v0.2.0",
		pinKeyNetworkProbe:    testTagV1,
	}
	if _, err := w.ValidateCreate(context.Background(), c); err != nil {
		t.Errorf("expected complete pinned map to validate, got: %v", err)
	}
}

func TestValidateCreate_ImagesReleaseStreamIgnoresPinned(t *testing.T) {
	w := newWebhook(t)
	c := newFixtureCluster("a")
	c.Spec.Images.Stream = ImageStreamRelease
	// Pinned omitted on purpose — must NOT trigger the validation.
	if _, err := w.ValidateCreate(context.Background(), c); err != nil {
		t.Errorf("release stream should not require pinned map, got: %v", err)
	}
}
...
func TestValidateCreate_ExternalEndpointAcceptedForAllFourServices(t *testing.T) {
	...
	mutators := map[string]func(*OpenCHAMIControlPlane){
		"smd": func(c *OpenCHAMIControlPlane) {
			c.Spec.Services.SMD.Enabled = false
			c.Spec.Services.SMD.ExternalEndpoint = &external
		},
		"tokensmith": func(c *OpenCHAMIControlPlane) {
			c.Spec.Services.Tokensmith.Enabled = false
			c.Spec.Services.Tokensmith.ExternalEndpoint = &external
		},
		"bootService": func(c *OpenCHAMIControlPlane) {
			c.Spec.Services.BootService.Enabled = false
			c.Spec.Services.BootService.ExternalEndpoint = &external
		},
		"metadataService": func(c *OpenCHAMIControlPlane) {
